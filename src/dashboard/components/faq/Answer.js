...
const Answer = () => {
  ...
  useEffect(() => {
    const fetchInquiry = async () => {
      try {
        const inquiryResponse = await fetch(`http://localhost:8000/inquiry/${question_no}`);
    ...
    const fetchAnswer = async () => {
      try {
        const answerResponse = await fetch(`http://localhost:8000/answer/${question_no}`);
  ...
  const handleSaveOrUpdate = async () => {
    ...
    try {
      ...
      const url = answer.length > 0
        ? `http://localhost:8000/answer/${question_no}` // 수정 시 question_no 사용
        : 'http://localhost:8000/answer'; // 등록 시 기본 URL 사용
  ...
  const handleDeactivate = async () => {
    try {
      const response = await fetch('http://localhost:8000/deactivate_inquiry', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
